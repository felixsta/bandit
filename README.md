...
для "Уровня <X>" содержат информацию о том, как начать уровень X с предыдущего уровня. Например, страница для Уровня 1 содержит информацию о том, как получить доступ с Уровня 0 на Уровень 1. Все уровни в этой игре имеют свою страницу на этом веб-сайте, и все они связаны со сбоку этой страницы.

Вы столкнетесь с множеством ситуаций, в которых вы не знаете, что вам делать. Не паникуйте! Не сдавайтесь! Цель этой игры - научить вас основам. Частью изучения основ является чтение большого количества новой информации. Если вы никогда раньше не использовали командную строку, хорошее введение - это введение в команды пользователя.
...
Примечание для виртуальных машин: Вам может не удаваться подключиться к overthewire.org через SSH с ошибкой "broken pipe", когда сетевой адаптер для виртуальной машины настроен на использование режима NAT. Добавление настройки IPQoS throughput в /etc/ssh/ssh_config должно устранить проблему. Если это не решит вашу проблему, единственным вариантом будет изменение адаптера на режим Bridged."
